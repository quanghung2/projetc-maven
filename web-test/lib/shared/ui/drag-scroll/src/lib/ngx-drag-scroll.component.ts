...
export class DragScrollComponent implements OnDestroy, AfterViewInit, OnChanges, AfterViewChecked {
  ...
  scrollTimer: number | any = -1;

  scrollToTimer: number | any = -1;
  ...
  ngAfterViewInit() {
    ...
    this.displayType =
      typeof window !== 'undefined' ? window.getComputedStyle(this._elementRef.nativeElement).display : 'block';
  ...
  private refreshWrapperDimensions() {
    if (this.wrapper) {
      ...
      this._renderer.setStyle(
        this.wrapper,
        'height',
        this._elementRef.nativeElement.style.height || this._elementRef.nativeElement.offsetHeight + 'px'
      );
    }
  }
  ...
  private locateCurrentIndex(snap?: boolean) {
    this.currentChildWidth((currentChildWidth, nextChildrenWidth, childrenWidth, idx: number, stop) => {
      if (
        ...
        if (
          nextChildrenWidth - this._contentRef.nativeElement.scrollLeft > currentChildWidth / 2 &&
          !this.isScrollReachesRightEnd()
        ) {
          // roll back scrolling
          if (!this.isAnimating) {
