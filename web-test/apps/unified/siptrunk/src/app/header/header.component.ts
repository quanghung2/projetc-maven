...
export class HeaderComponent implements OnInit, OnDestroy {
  ...
  private initSipAccounts() {
    ...
    this.sipTrunkService.getAccounts(TypeSipAccount.SIP_TRUNK).subscribe(sips => {
    ...
    this.sipTrunkService.getAvailableCallerIds().subscribe();
  }
  ...
  private initLicense(sips: SipAccount[]) {
    ...
    this.licenseService
      ...
      .subscribe(([license, resources]: [License[], LicenseResource[]]) => {
        const updateSips: SipAccount[] = [];
        sips.forEach(sip => {
          const findMapping = license.find(x => x?.resource?.key === sip.sipUsername);
          if (findMapping) {
        ...
        sips
          .filter(x => x?.detail?.subscriptionUuid)
          .forEach(item => {
            const findLicense = license.find(x => x.subscriptionUuid === item.detail.subscriptionUuid);
            if (findLicense) {
              updateSips.push(
                new SipAccount({
                  ...item,
                  isIP: findLicense?.isSipIpAuthen
                })
              );
            }
          });
  ...
  private initSidebarData(profileOrg: ProfileOrg, _: string[]) {
    ...
    const menus: RouteMap[] = [
      MENU_ROUTE_MAPS?.['overview'],
      MENU_ROUTE_MAPS?.['config_detail'],
      MENU_ROUTE_MAPS?.['networking_info'],
      MENU_ROUTE_MAPS?.['security']
      // MENU_ROUTE_MAPS?.['high_availability']
    ];
