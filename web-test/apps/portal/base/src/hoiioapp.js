...
var Hoiio = {
  ...
  _initContext: function (env) {
    ...
    this.orgUuid = urlParams['orgUuid'];
    this.sessionToken = urlParams['sessionToken'];
  ...
  getCookieInfo: function () {
    ...
    if (window.location.hostname.startsWith('portal-uat')) {
      domain += 'UAT';
    }
  ...
  _putContext: function (sessionToken, orgUuid) {
    ...
    document.cookie =
      'session_' + this._getPortalDomain() + '=' + encodeURIComponent(JSON.stringify(session)) + ';path=/';
  },
  ...
  _addEventListener: function (eventName, onEvent, onceOnly) {
    ...
    this.listeners[eventName].push({ listenerId: listenerId, listener: onEvent, onceOnly: onceOnly });
    return listenerId;
  },
