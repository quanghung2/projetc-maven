...
import { InvoiceService, Template } from '../../core';
...
@Component({
  ...
  styleUrls: ['../shared/invoice.css', '../shared/styles.scss', './monthly-invoice.component.scss']
})
export class MonthlyInvoiceComponent implements OnInit {
  ...
  resetChanges() {
    ...
    this.dueNoteEditor.setContent(this.template.monthlyUnpaidNote);
    this.paidNoteEditor.setContent(this.template.monthlyPaidNote);
  ...
  onDueNoteChanged($event) {
    ...
    this.unsavedTemplate.monthlyUnpaidNote = $event;
  }

  onPaidNoteChanged($event) {
    this.hasChanges = true;
    this.unsavedTemplate.monthlyPaidNote = $event;
  }
  ...
  onCurrencyChanged(currency) {
    ...
    this.templateService
      ...
      .subscribe(template => {
        ...
        this.dueNoteEditor.setContent(template.monthlyUnpaidNote);
        this.paidNoteEditor.setContent(template.monthlyPaidNote);
      });
  }
}
