...
export class MessageReceiveProcessor {
  ...
  process(message: ChatMessage) {
    ...
    if (!!message.id && message.ts) {
      this.chatService.timeLastestMessageReceived = message.ts; // diff 10s make sure not miss msg - save time when disconnect ws
    }
  ...
  private processMsg(convo: ConversationGroup, message: ChatMessage) {
    ...
    if (message.isStore) {
      ...
      if (message.user !== customer.chatCustomerId) {
        const lastestReadMsgId = this.convoGroupQuery.getConvoUiState(convo.conversationGroupId)?.lastestReadMsgId;
        if (convo.unreadCount === 0 && !lastestReadMsgId) {
          this.convoGroupService.updateConvoViewState(convo.conversationGroupId, <ConversationGroupUI>{
            lastestReadMsgId: message.id
          });
        }
