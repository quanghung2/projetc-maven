...
import { ProfileQuery } from '@b3networks/api/auth';
...
import { deltaHasContent, DestroySubscriberComponent, getCountryTimeZoneLocal } from '@b3networks/shared/common';
import { ToastService } from '@b3networks/shared/ui/toast';
import { format } from 'date-fns-tz';
import { QuillEditorComponent, QuillModules } from 'ngx-quill';
import { Delta, Quill } from 'quill';
import { fromEvent, Observable, Subscription, timer } from 'rxjs';
import { debounceTime, filter, finalize, map, share, takeUntil, tap } from 'rxjs/operators';
...
export class ChatboxContentComponent extends DestroySubscriberComponent implements OnInit, OnChanges, AfterViewInit {
  ...
  @ViewChildren('item', { read: ElementRef }) messageCmps: QueryList<ElementRef>;

  @Input() livechat: ConversationGroup;
  @Input() ui: UIConfig;
  @Input() hasChatBox: boolean;
  @Input() animation: boolean;
  @Input() enableIndividualMetting: boolean;
  ...
  onIndividualMetting() {
    const customer = this.customersQuery.getValue();
    const agent = this.profileQuery.getValue();
    this.chatbotService
      .startIndividualMeeting(<ReqIndividualMetting>{
        org_uuid: customer.orgUuid,
        identity_uuid: agent.iid,
        chat_customer_id: customer.chatCustomerId,
  ...
  private loadImage(entry: IntersectionObserverEntry) {
    const mediaUuid = entry.target.getAttribute('uuid');
    const convoUuid = entry.target.getAttribute('convo');

    this.mediaService.getMediaImgThumbnailPublic(mediaUuid, convoUuid).subscribe(res => {
      entry.target.innerHTML = ''; // clear mat-spiner
      (<HTMLElement>entry.target).style.backgroundImage = `url('${res.url}')`;
      entry.target.dispatchEvent(new Event('load')); // loaded
    });
  }

  private trackingLazyLoadingImg() {
    if (this.viewport) {
      ...
      this.preLoadImage(this.messageCmps.toArray());

      this.messageCmps.changes
        .pipe(takeUntil(this.destroySubscriber$), debounceTime(500))
        .subscribe((items: QueryList<ElementRef>) => {
          this.preLoadImage(items);
        });
    } else {
      setTimeout(() => {
        this.trackingLazyLoadingImg();
  ...
  private preLoadImage(items: QueryList<ElementRef> | ElementRef[]) {
    if (this.viewport && items.length > 0) {
      items.forEach((e: ElementRef) => {
        const hasImg = e.nativeElement.querySelector('.fake-image');
        if (hasImg) {
          hasImg.classList.remove('.fake-image');
          setTimeout(() => {
            this.intersectionObserver.observe(hasImg);
          }, 0);
        }
      });
    }
